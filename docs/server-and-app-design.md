...
> **【文档关系 · 2026-07-23】**
> 本文描述 **数据面** 服务器化 / Web / App（主播、导入导出、仪表盘）。
> **微信机器人 · AI Agent · RAG · Bot Worker** 的产品范围以 `docs/ai-agent-production-plan.md` 为准，跨模块运行契约以 `docs/adr/0001-agent-runtime-contract.md` 为准。
> 冲突时：业务日默认 **昨天**、CSV 导入默认昨天（不读文件名）、智能模式业务命令关闭、同一微信账号仅一个 runner，以 Agent 生产计划与 ADR 为准。
> 「今日数据概览」等表述指 **最新业务日数据**，不是强制日历今天。
