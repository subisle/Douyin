...
## 11. Web UI：在浏览器里管理机器人与数据（2026-09-18）

盒子上跑的就是「桌面同一套 UI」，浏览器直接打开即可，不需要 Electron：

```
http://192.168.0.13:3000/
```

用 `.env` 里的 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 登录（会话走 httpOnly Cookie）。
前端所有数据都走 `/api/v1/*`，所以桌面与浏览器是同一份实现，功能一致。
...
**微信（iLink）**
- 状态徽标：未连接 / 待扫码 / 运行中 / 登录过期 / 异常
- 「扫码登录」按钮 → 页面直接显示二维码（`data:image/png;base64,...`）→ 手机微信扫
- 账号列表：多账号可切换当前账号、启动/暂停、断开
- 「命令能力」页签：指令说明、自定义指令、自动回复
- 「推送提醒」页签：日报自动推送开关与状态
- 消息日志：用户消息与机器人回复

**QQ（开放平台）**
- 「机器人实例」卡片：列出全部实例（多机器人），每个显示 `label / AppID / 状态 / 消息数`，
  右侧按钮按实例 **连接 / 断开**；点实例名切换下方凭证、设置与消息日志的作用对象
- 凭证卡片：AppID / ClientSecret / apiBase / intents / 自动连接 / 自动回复
- 消息日志：群 @ 与私聊消息

> 新增 QQ 机器人仍然走配置文件（见 10.2 / 10.7）：一份 `qq-bots/*.json` 一个实例，
> 重启容器后出现在实例列表里，可直接在页面上连接，不需要改代码。

### 11.2 权限与安全

- 所有 `/api/v1/*` 都要求登录会话或 `API_TOKENS`；未登录只会看到登录页
- 登录失败有速率限制（`login-rate-limit`），连续失败会 429
- 设置接口会**返回 ClientSecret 原值**（桌面端要能编辑），所以不要把这个端口暴露到公网；
  需要远程访问请套一层反向代理 + 额外认证，或只放局域网

### 11.3 常见问题

| 现象 | 原因 / 处理 |
| --- | --- |
| 页面能开，机器人卡片显示「登录过期」 | 服务器无法解密桌面的 token，点「扫码登录」在盒子上重新扫一次（见 10.6） |
| QQ 实例显示 `未配置 AppID` | 检查 `qq-bots/*.json` 里 `appId`/`clientSecret` 是否在顶层或 `settings` 下；启动日志会写明跳过原因 |
| 实例列表为空 | 容器没以 `PROJECT_BOTS=1` 启动，或 `builtin` 卷没挂上；看日志里的 `[project-bots] 诊断` 行 |
| 改了配置不生效 | 配置在启动时读取，改完要 `docker compose up -d` 重建容器 |
