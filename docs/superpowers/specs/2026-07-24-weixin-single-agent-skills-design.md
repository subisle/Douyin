...
> **【已废弃 · 2026-09-18】**
>
> 本项目已在 `3318` 分支**彻底移除 AI 能力**（单 Agent、技能工具调用、RAG、会话记忆全部删除）。
> 本文档仅作历史存档，**不再代表产品形态**；当前约束以 `CLAUDE.md` 第 2 节为准：
> 机器人只提供固定指令 + 确定性技能，禁止重新引入模型调用。
...
| 审计依据 | 2026-07-24 代码审计 + 2026-07-27 进度回执/用户记忆落地：`electron/weixin-bot*.js`、`electron/main.js` |
| 进度旁路 | 服务器实验见 `docs/ilink-implementation-progress.md`（**非本规格主路径**） |
| 扩展规格 | 进度回执 `2026-07-27-weixin-agent-progress-status-design.md`；用户记忆 `2026-07-27-weixin-agent-user-memory-design.md` |
...
1. 微信可完成：帮助、查艺名数据、日报图、CSV（AI 对话或固定指令兜底）。  
2. 配置 AI 后默认直接对话，全部业务文本可触发技能并返回带数据截止日期的回答。  
3. 未配置 AI 时，固定指令兜底仍可用；Agent 明确不处理（不假装成功）。  
4. 规格与代码技能表、默认 AI 路径 **无矛盾**。
...
createWeixinCommandHandler   electron/weixin-bot-commands.js
  │  系统口令 / CSV / AI 就绪分流
  ├─ AI 未就绪 → parseBotCommand → analytics/命令执行（兜底）
  └─ AI 就绪 → handled:false → agentHandler
              ▼
...
| 路径 | 默认 | 进入 | 退出 | 行为 |
| --- | --- | --- | --- | --- |
| `agent`（智能对话） | **是**（AI 已启用时） | 默认；「人工客服」仅显式确认 | 无「退回指令模式」；「清空对话 / 退出客服」只清记忆 | **全部业务文本进 Agent + 技能**（含原固定指令说法） |
| 固定指令兜底 | 否；仅 AI 未就绪 | AI 未配置/未启用时自动 | 配置并启用 AI 后 | 固定命令；**不调用 LLM** |

实现：`createModeStore({ defaultMode: "agent" })`（`weixin-bot-mode.js`），按 `accountId + 用户/群` 分会话；TTL 约 2h。  
**AI 未就绪时**命令层仍走固定指令兜底，保证「每日报告」等可用。  
**产品无独立「指令模式」入口，也无 FastRoute 业务旁路**；AI 就绪后文本一律对话。

### 3.2 系统口令（优先于业务）

| 类型 | 示例 | 行为 |
| --- | --- | --- |
| 帮助 | 帮助、菜单、命令、`/help` | 返回智能对话说明 |
| 确认智能 | 人工客服、智能客服、客服… | 确认 AI 就绪；`setMode(agent)`（默认已是） |
| 清空记忆 | 退出客服、关闭客服、清空对话… | **保持 agent** + 清线程（不退回指令路径） |

### 3.3 产品含义（必须写进用户说明）

- **默认直接对话**（桌面 AI 开关打开并保存后）：用户不必先发「人工客服」，也没有「指令模式」可切。  
- **全部业务文本走 AI**：如「每日报告」、艺名、导出等说法也由模型选技能执行（数字仍只来自工具）。  
- CSV 文件导入仍确定性（非文本旁路）。  
- AI 未配置或未启用时自动固定指令兜底，避免无法办事。
...
2. **系统口令**（帮助 / 开客服 / 退客服）  
3. **CSV / 文件导入**（若适用）  
4. 读取有效路径（AI 就绪 → agent；否则固定指令兜底）  
5. **智能对话：** commandHandler **不**执行业务指令；返回 `handled: false` 交给 `agentHandler`  
6. **固定指令兜底（AI 未就绪）：** 自定义命令 → `parseBotCommand` → 执行  
7. 若仍未 handled 且存在 `agentHandler` → `WeixinBotAgent.handleMessage`  
8. Agent 内：未配置 → `handled: false`；默认会话可进 Agent → **LLM tool loop**（不再 Agent 内 FastRoute）  

**确定性优先：** 仅 CSV 文件导入与 AI 未就绪时的固定指令兜底保持确定性；AI 就绪后业务文本全部走模型+技能。
...
| 超时 | 默认 90s 级，可配（`AI_TIMEOUT_MS` / 设置项，硬顶 120s） | 单次 completion（含 reasoning 模型 tool-call） |
...
- AI 未就绪时通过 `parseBotCommand` + 命令处理函数走 **同一 analytics 能力**。  
- 智能对话通过 **skills 表** 暴露给模型。  
...
**位置：** `matchFastRoute` 仍保留于 `weixin-bot-mode.js`，供单元测试与兼容导出。  
**产品行为：** AI 就绪时 **不再** 用 FastRoute 拦截「每日报告 / 艺名」等文本；一律进 Agent。  
**仍确定性：** CSV 入站导入、系统口令（帮助/清空记忆）、AI 未就绪时的固定指令兜底。
...
### 11.1 已落地扩展能力（交叉引用，不改变单 Agent 冻结）

以下为 **同一** `WeixinBotAgent` 上的体验/记忆增强，**不是**新 Agent 或新用户通道：

| 能力 | 规格 | 代码锚点 | 说明 |
| --- | --- | --- | --- |
| 进度回执 | `2026-07-27-weixin-agent-progress-status-design.md` | `weixin-bot-agent.js`（开场 + 工具进度；`progressEnabled` 默认开可关） | Hermes 风格轻量版；fail-open |
| AI 超时默认 90s | （同上规格上下文 / 设置页） | `DEFAULT_TIMEOUT_MS = 90_000`；`AI_TIMEOUT_MS` 可覆盖 | 适配慢推理模型 |
| 用户记忆 / 习惯一期 | `2026-07-27-weixin-agent-user-memory-design.md` | `weixin-bot-user-memory.js` + Agent 注入/学习 | 线程落盘加长；成功技能自动学习惯；「清空对话」只清线程 |
...
- `npm run test:weixin-bot` — 命令、mode、agent、AI 全量路径相关测  
- 技能逻辑多经 analytics / commands 测覆盖  

### 12.2 实机验收清单（桌面）

- [ ] 未配 AI：固定指令「帮助」「艺名」「每日报告」可用  
- [ ] 配 AI 后无需先发「人工客服」：直接自然语言对话  
- [ ] 自然语言查音浪/时长：调用技能，数字与库一致或明确无数据  
- [ ] 要日报图：收到图片而非纯空话  
- [ ] 「退出客服 / 清空对话」：只清记忆，仍保持智能对话  
- [ ] 无关问题：拒绝或不编造业务数字  
...
| 模式 / 会话键 / 兼容 FastRoute 工具函数 | `electron/weixin-bot-mode.js` |
| 单 Agent（含进度回执、超时） | `electron/weixin-bot-agent.js` |
| 用户记忆 / 习惯一期 | `electron/weixin-bot-user-memory.js` |
...
| 2026-07-27 | v1.1.0：交叉引用进度回执 / 用户记忆扩展规格；代码锚点补 user-memory；**不**推翻单 Agent 冻结 |
